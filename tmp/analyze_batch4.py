...
for conv in conversations:
                        ...
                        if turns[j]['speaker'] == 'AI':
                            ...
                            if ('这家企业是' in ai_resp or '岗位是' in ai_resp or '要不要接触' in ai_resp or '简历' in ai_resp or '给企业HR' in ai_resp):
                                violations.append({
                                    ...
                                    'suggestion': 'AI在用户明确表达拒绝意愿后，应立即礼貌结束对话并告别，而非继续播报岗位信息、追问是否接触或推进要简历流程。'
                                })
                                r01_found = True
                            break
    ...
    if user_has_substantial_response(conv):
        ...
        if last_ai and not has_farewell and len(last_ai) > 10 and '您是' not in last_ai:
            violations.append({
                'rule_id': 'R02',
                'rule_name': '对话未正常结束',
                'severity': '高',
    ...
    for i, t in enumerate(turns):
        ...
        if t['speaker'] == '用户':
            txt = t['text']
            if any(p in txt for p in ['开会','不方便','在忙','稍等','等一下','现在忙','稍后','我现在在忙','我现在不太方便','我还没看呢，还在忙']):
                for j in range(i+1, len(turns)):
                    if turns[j]['speaker'] == 'AI':
                        ai_resp = turns[j]['text']
                        has_followup = bool(re.search(r'(明天|后天|下周|改天|下午|上午).*联系', ai_resp))
                        has_goodbye = any(kw in ai_resp for kw in ['再见','不打扰','先忙'])
                        if not has_followup:
                            if has_goodbye:
                                violations.append({
                                    'rule_id': 'R08',
                                    ...
                                    'severity': '中',
                                    'evidence': '用户: ' + txt + ' | AI: ' + ai_resp,
                                    'suggestion': 'AI在用户表示不便时，仅简单告别而未约定下次联系时间，应主动提出具体联系时间。'
                                })
                                found_r08 = True
                            elif '简历' in ai_resp or 'HR' in ai_resp:
                                violations.append({
                                    'rule_id': 'R08',
                                    'rule_name': '用户不便时未妥善处理',
                                    'severity': '中',
                                    'evidence': '用户: ' + txt + ' | AI: ' + ai_resp,
                                    'suggestion': 'AI在用户表示不便时未结束通话并约定时间，而是继续推进流程。'
                                })
                                found_r08 = True
                        break
    ...
    for i, t in enumerate(turns):
        ...
        if t['speaker'] == '用户':
            txt = t['text']
            if any(p in txt for p in ['可以','好的','行行','同意']) and 'HR' not in txt:
                user_agreed = True
...
# Save to temp for building final JSON
with open('C:/Users/dongyi/myprojects/auto-qc/tmp/results_analysis.json', 'w', encoding='utf-8') as f:
