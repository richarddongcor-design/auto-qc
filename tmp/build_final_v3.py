...
# Helper: find the user text that immediately precedes an AI turn
def find_preceding_user_text(turns, ai_idx):
    for i in range(ai_idx - 1, -1, -1):
        if turns[i]['speaker'] == '用户':
            return turns[i]['text']
    return '无'
...
for conv in conversations:
    ...
    if user_has_substantial_response(conv):
        ...
        if last_ai and not has_farewell and not ends_with_question and len(last_ai) > 10 and '您是' not in last_ai:
            last_user = find_preceding_user_text(turns, len(turns) - 1)
            violations.append({
                'rule_id': 'R02',
                'rule_name': '对话未正常结束',
                'severity': '高',
                'evidence': '用户: ' + last_user + ' | AI: ' + last_ai,
    ...
    for i in range(len(cleaned_ai)):
        ...
        for j in range(i+1, len(cleaned_ai)):
            t1, c1 = cleaned_ai[i]
            t2, c2 = cleaned_ai[j]
            if c1 and c2 and len(c1) > 20 and c1 == c2:
                # Find preceding user text for context
                ai_idx_in_turns = None
                for idx, tt in enumerate(turns):
                    if tt['speaker'] == 'AI' and tt['text'] == t1:
                        ai_idx_in_turns = idx
                        break
                user_ctx = find_preceding_user_text(turns, ai_idx_in_turns) if ai_idx_in_turns else ''
                violations.append({
                    'rule_id': 'R03',
                    'rule_name': 'AI内容重复播报',
                    'severity': '高',
                    'evidence': '用户: ' + user_ctx + ' | AI重复播报: ' + t1[:80] + ' | AI再次播报: ' + t2[:80],
            ...
            if company and location:
                ...
                if company_city:
                    ...
                    if loc_city and company_city != loc_city:
                        user_ctx = find_preceding_user_text(turns, idx)
                        violations.append({
                            'rule_id': 'R04',
                            'rule_name': '公司名与工作地点矛盾',
                            'severity': '高',
                            'evidence': '用户: ' + user_ctx + ' | AI: 公司名含' + company_city + ' (' + company + ') 但工作地点在' + location,
                            'suggestion': 'AI播报时公司名包含城市标识"' + company_city + '"但实际工作地点在"' + loc_city + '"，应区分说明公司注册地与实际工作地。'
                        })
                        r04_found = True
    ...
    for t in ai_texts:
        for pat in confirm_patterns:
            if pat in t:
                ...
                break
    if confirm_count >= 3:
        # Find first AI turn with confirm for user context
        first_ai_idx = None
        for idx, tt in enumerate(turns):
            if tt['speaker'] == 'AI':
                for pat in confirm_patterns:
                ...
                if first_ai_idx:
                    break
        user_ctx = find_preceding_user_text(turns, first_ai_idx) if first_ai_idx else ''
        violations.append({
            'rule_id': 'R06',
            'rule_name': '重复确认意向',
            'severity': '高',
            'evidence': '用户: ' + user_ctx + ' | AI在对话中' + str(confirm_count) + '次重复确认意向: ' + '; '.join(confirm_examples[:3]),
        ...
        if t['speaker'] == '用户' and t['text'].strip() == '用户无应答':
            ...
            if t['speaker'] == 'AI' and user_no_response_streak >= 2:
                if '这家企业是' in t['text'] or '要不要接触' in t['text'] or '岗位' in t['text'] or '您看' in t['text']:
                    prev_user = '用户无应答(连续' + str(user_no_response_streak) + '次)'
                    violations.append({
                        'rule_id': 'R07',
                        'rule_name': '无响应时循环追问',
                        'severity': '中',
                        'evidence': '用户: ' + prev_user + ' | AI在连续' + str(user_no_response_streak) + '次无应答后继续播报: ' + t['text'][:80],
        ...
        if t['speaker'] == 'AI' and ('信号不太好' in t['text'] or '听不见您说话' in t['text']):
            ...
            if has_user_response:
                violations.append({
                    ...
                    'rule_name': '误判用户信号',
                    'severity': '中',
                    'evidence': '用户: ' + recent_user_text + ' | AI: ' + t['text'],
    ...
    for idx, t in enumerate(ai_texts):
        ...
        if ('明白' in t or '哦/' in t) and ('再见' in t or '联系您' in t):
            if check_r10(t):
                # Find the turn index in turns
                turn_idx = None
                for ti, tt in enumerate(turns):
                    if tt['speaker'] == 'AI' and tt['text'] == t:
                        turn_idx = ti
                        break
                user_ctx = find_preceding_user_text(turns, turn_idx) if turn_idx else ''
                violations.append({
                    'rule_id': 'R10',
                    'rule_name': '结束话术出现乱码',
                    'severity': '中',
                    'evidence': '用户: ' + user_ctx + ' | AI结束话术: ' + t,
    ...
    for i in range(len(turns)-1):
        ...
        if turns[i]['speaker'] == 'AI' and turns[i+1]['speaker'] == 'AI':
            ...
            if c1 and c2 and c1 == c2:
                # Find user text before the first AI mention
                user_ctx = '无'
                for k in range(i-1, -1, -1):
                    if turns[k]['speaker'] == '用户':
                        user_ctx = turns[k]['text']
                        break
                violations.append({
                    'rule_id': 'R11',
                    'rule_name': '公司自我介绍冗余重复',
                    'severity': '中',
                    'evidence': '用户: ' + user_ctx + ' | AI连续两轮提及同公司: ' + turns[i]['text'][:60] + ' | ' + turns[i+1]['text'][:60],
    ...
    for i, t in enumerate(turns):
        ...
        if t['speaker'] == 'AI' and user_agreed:
            if '简历' in t['text'] and ('HR' in t['text'] or '企业' in t['text']):
                # Find preceding user text
                user_ctx = find_preceding_user_text(turns, i)
                violations.append({
                    'rule_id': 'R13',
                    'rule_name': '用户同意后重复要简历',
                    'severity': '低',
                    'evidence': '用户: ' + user_ctx + ' | AI: ' + t['text'][:80],
...
# Build spot_check_details
spot_checks = []

spot_checks.append({
    'id': '11228776342',
    'reasoning': '逐条检查所有13条规则：R01: 用户未明确拒绝，无触发。R02: AI最后一条发言包含"祝您生活愉快，再见"，正常结束。R03: AI各轮发言内容不同，无重复。R04: 公司名"恒华数字科技集团有限公司"不含城市标识，地点"北京朝阳区"，无矛盾。R05: 用户问"主要工作以设计为主是吗还是说以优化什么的为主"，这是对岗位具体工作内容的追问，AI未给出任何实质性回答，直接跳转到"呃好，那稍后我把您的简历还有联系方式呢给企业HR看一下"，完全回避了用户的问题，触发R05。R06: AI确认意向仅2次（要不要接触、您看可以吗），未达3次阈值。R07: 无连续无应答场景。R08: 用户未表示不便。R09: 无信号误判。R10: 结束语格式正常。R11: 无连续两轮同公司介绍。R12: 用户未要求在平台查看。R13: 用户说"行，可以可以可以"后AI没有重复要简历。最终判定：violation (R05)。'
})

spot_checks.append({
    'id': '11189265895',
    'reasoning': '逐条检查所有13条规则：R01: 用户未明确拒绝。R02: AI最后发言包含"再见"，正常结束。R03: 各轮AI发言不同。R04: 公司名无城市标识，地点"青岛城阳区"，一致。R05: 用户无具体追问岗位细节。R06: AI发出4次确认意向提问（要不要接触、您看可以吗、你们详细沟通下怎么样、您看行吗），超过2次阈值，触发R06。R07: 用户有发言，不属于无应答。R08: 用户说"我现在在忙啊"表示在忙，AI没有约定下次联系时间，而是继续追问确认意向，触发R08。R09: AI说"信号不太好听不见您说话"，但此前用户有明确发言"一会儿吧我看完再说吧"，属于误判，触发R09。R10: 结束语格式正常。R11: 无连续两轮同公司。R12-R13: 无触发。最终判定：violation (R06, R08, R09)。'
})

spot_checks.append({
    'id': '11195799789',
    'reasoning': '逐条检查所有13条规则：R01: 用户未明确拒绝。R02: 用户有实质发言但AI最后一条是岗位介绍无告别词，通话中断，触发R02。R03: AI在两个不同轮次中完整重复播报相同岗位介绍，触发R03。R04: 公司名无城市标识，地点苏州太仓市，无矛盾。R05-R13均无触发。最终判定：violation (R02, R03)。'
})

spot_checks.append({
    'id': '11115944326',
    'reasoning': '逐条检查所有13条规则：R01: 用户未拒绝。R02: AI最后发言含告别，正常结束。R03: 无重复。R04: 无矛盾。R05: 用户无具体追问。R06: AI发出4次确认意向提问，超过2次阈值，触发R06。R07: 无连续无应答。R08: 用户未表示不便。R09: 无信号误判。R10: 结束语正常。R11: 无相邻两轮同公司。R12: 用户说"我先看一下你们那个在平台上发的信息"，AI仍推送将简历给HR，触发R12。R13: 用户说"可以可以"同意后AI仍问"您看可以吗"要简历，触发R13。最终判定：violation (R06, R12, R13)。'
})

spot_checks.append({
    'id': '11191692883',
    'reasoning': '逐条检查所有13条规则：R01: 用户未拒绝。R02: AI最后发言含告别，正常结束。R03: AI在两个轮次中重复播报"呃您好，要么我先把您简历给企业HR看看，合适的话企业HR就直接联系您，您看行吗"，触发R03。R04: 无矛盾。R05: 用户追问"公司是多少人的公司啊"，AI用"已在猎聘发送"搪塞但后面确实提供了公司信息，暂不标记R05。R06: AI在对话中至少7次重复确认意向，远超阈值，触发R06。R07: 无连续无应答。R08: 用户说"稍等一下"属于等待查看，非真正不便。R09-R13: 无触发。最终判定：violation (R03, R06)。'
})
...
# Check all violations have evidence with | separator
for r in output['results']:
    for v in r['violations']:
        assert 'rule_id' in v and 'rule_name' in v and 'severity' in v and 'evidence' in v and 'suggestion' in v
        assert ' | ' in v['evidence'], f'{r["id"]} {v["rule_id"]}: evidence missing |: {v["evidence"][:80]}'

for s in output['spot_check_details']:
    assert 'id' in s and 'reasoning' in s

print('All evidence has | separator: PASS')
